export const vertexShader_luminanceGlitch = `
varying vec2 vUv;
void main() {
    vUv = uv;
    gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
}
`

export const fragmentShader_luminanceGlitch = `
uniform sampler2D u_tex;
uniform float u_time;
uniform float u_glitchStrength;
uniform vec2  u_resolution;

varying vec2 vUv;

void main()
{
    //Normalized screen uvs [0, 1]
    vec2 uv = vUv;
    float x = uv.s;
    float y = uv.t;
    
    float glitchStrength = max(u_glitchStrength, 0.15); // multiplier pour l'intensité globale du glitch
    
    
    // get snapped position
    float psize = 0.04 * glitchStrength;
    float psq = 1.0 / psize;

    float px = floor( x * psq + 0.5) * psize;
    float py = floor( y * psq + 0.5) * psize;
    
	vec4 colSnap = texture( u_tex, vec2( px,py) );
    
	float lum = pow( 1.0 - (colSnap.r + colSnap.g + colSnap.b) / 3.0, glitchStrength ); // remove the minus one if you want to invert luma
    
    
    
    // do move with lum as multiplying factor
    float qsize = psize * lum;
    
    float qsq = 1.0 / qsize;

    float qx = floor( x * qsq + 0.5) * qsize;
    float qy = floor( y * qsq + 0.5) * qsize;

    float rx = (px - qx) * lum + x;
    float ry = (py - qy) * lum + y;
    
	vec4 colMove = texture( u_tex, vec2( rx,ry) );
    
    
    // final color
    gl_FragColor = colMove;
}
`
